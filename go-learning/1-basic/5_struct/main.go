...
// ScaleBy
//只要 receiver 是值类型的，无论是使用值类型的实例还是指针类型的实例，都是拷贝整个底层数据结构的，方法内部访问的和修改的都是实例的副本。
//   所以，如果有修改操作，不会影响外部原始实例。
//func (p Point) ScaleBy(factor float64) {
//	p.X *= factor
//	p.Y *= factor
//}
//只要 receiver 是指针类型的，无论是使用值类型的实例还是指针类型的实例，都是拷贝指针，方法内部访问的和修改的都是原始的实例数据结构。所以，如果
//   有修改操作，会影响外部原始实例。
func (p *Point) ScaleBy(factor float64) {
	p.X *= factor
	p.Y *= factor
...
//Go 函数给参数传递值的时候是以复制的方式进行的，所以为了避免复制的开销，以及函数中的修改对原有值有效，大多数时候，传递给函数的数据结构都是它们的指针
...
func main() {
	...
	//具名嵌套结构体赋值方式、调用其嵌套 struct 的成员、函数
	s7 := Circle{
		Radius: 1,
		P: &Point{
			X: 1,
