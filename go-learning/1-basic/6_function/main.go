...
// 3. Go中不允许函数重载(overload)，也就是说不允许函数同名。
...
//错误使用：捕获了迭代变量
// 变量d在for循环引进的一个块作用域内进行声明。在循环里创建的所有函数变量共享相同的变量--一个可访问的存储位置，而不是固定的值。
// d变量的值在不断地迭代中更新，因此当调用清理函数时，d变量已经被每一次的for循环更新多次。因此，d变量的实际取值是最后一次迭代时的值，所以输出结果均为5。
...
//第一种方法是在循环体内部再定义一个局部变量，这样每次迭代printFuncs语句的闭包函数捕获的都是不同的变量，这些变量的值对应迭代时的值。
...
//第二种方式是将迭代变量通过匿名函数的参数传入，printFuncs语句会马上对调用参数求值。
