...
//Go中函数的参数是按值传递的，而slice是引用类型，将slice 传递给函数，实际上将{ptr, capacity, len}进行了拷贝，
//  所以和源slice指向同一个底层数组。换句话说，如果函数内部对slice进行了修改，有可能会直接影响函数外部的底层数组，从而影响其它slice。
// 通常需要将原有slice作为函数返回值。
...
func main() {
	...
	//1.3 在Go中，当一个变量被声明之后，都会立即对其进行默认的赋0初始化。
	//对int类型的变量会默认初始化为0，对string类型的变量会初始化为空""，对布尔类型的变量会初始化为false，对指针(引用)类型的变量会初始化为nil。
	var a3 [5]*int
	fmt.Println(a3)
	...
	//  的第一个元素并不一定就是数组的第一个元素），长度对 应slice 中元素的数目，容量一般是从 slice 的开始位置到底层数据的结尾位置，长度不能超过容量。
	//|---------------|---------|----------|
	//| 0xc00007dd70  |   3     |     5    |
	//|      ptr      | Length  | Capacity |
	//|---------------|---------|----------|
	...
	//2.1 切片的基本定义与初始化、遍历
	var s0 = []int{1, 2, 3}
	//s0 := []int{1, 2, 3}          //直接赋值初始化的方式创建slice
	for i, val := range s0 {
		fmt.Println(i, val)
	}
	...
	//|  ptr   | Length  | Capacity |
	//|--------|---------|----------|
	var s2 []int // 声明一个nil slice
	println(s2)  //[0/0]0x0
	//空 slice
	//虽然nil slice和 Empty slice 的长度和容量都为0，输出时的结果都是[]，且都不存储任何数据，但它们是不同的。
	...
	s44 := append(s4, 4)
	fmt.Println(len(s4), cap(s4), s4)    // 3 3 [1 2 3] --- 原有 s4 的len和capacity不变，实际上底层数组的长度和capacity变化了
	fmt.Println(len(s44), cap(s44), s44) // 4 6 [1 2 3 4]  --- s44 的len和capacity变化了，

	//2.5 对 slice 进行切片 --- 进行的是浅拷贝
	//SLICE[A:B] 截取时"左闭右开"，提取[A, B)区间元素，新的capacity = 原capacity - A
	// SLICE[A:]  // 从A切到最尾部
	...
	s13 := []int{1, 2, 3}
	s14 := make([]int, 2, 5)
	copy(s14, s13)   //注意：只会拷贝到目标切片的len长度，超过的丢弃
