...
func stringExample() {
	//string --- 值类型
	// 1. Go中的string类型要使用双引号或反引号包围，它们的区别是：
	...
	//2. string的底层是byte数组，每个string其实只占用两个机器字长：一个指针和一个长度。只不过这个指针在Go中完全不可见，所以对我们来说，
	//    string是一个底层byte数组的值类型而非指针类型。
	//所以，可以将一个string使用copy()拷贝到一个给定的byte slice中，也可以使用slice的切片功能截取string中的片段。
	var a = "hello world!"
	//使用slice的切片功能、len函数获取长度
	println(a[2:4], len(a)) //ll 12
	s1 := make([]byte, 12)
	//使用copy函数，将一个string拷贝到一个给定的byte slice中
	copy(s1, a)
	...
	//5. 字符串串接
	//使用加号+连接两段字符串，字符串连接+操作符强制认为它两边的都是string类型，所以 "abc" + 2 将报错。需要先将int类型的2转换为字符串
	//   类型(不能使用string(2)的方式转换，因为这种转换方式不能跨大类型转换，只能使用strconv包中的函数转换)。
	//另一种更高效的字符串串接方式是使用strings包中的Join()函数，它可以在缓冲中将字符串串接起来。
	s4 := "first" + "," + "second"
	println(s4)

	s5 := []string{"foo", "bar", "baz"} // foo, bar, baz
	fmt.Println(strings.Join(s5, ", "))

	//6.字符串和数字的互相转换
	//• 要将整数转换成字符串，一种选择是使用fmt.Sprintf，另一种选择是strconv.Itoa函数。
	//• strconv包内的 Atoi函数或 ParseInt 函数用于解释表示整数的字符串。
