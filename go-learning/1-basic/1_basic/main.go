...
func main() {
	...
	//• 接口类型(interface type)：
	//当声明变量的时候，会做默认的赋0初始化。每种数据类型的默认赋0初始化的0值不同，例如int类型的0值为数值0，float的0值为0.0，string类型
	//  的0值为空""，bool类型的0值为false，数据结构的0值为nil，struct的0值为字段全部赋0。
	...
	*pInt = 2
	...
	//3.new 和 make 内存分配
	//new 是一个分配内存的内建函数，但不同于其他语言中同名的new所作的工作，它只是将内存清零，而不是初始化内存。new(T)为一个类型为T的新项目
	//  分配了值为零的存储空间并返回其地址，也就是一个类型为*T的值。用Go的术语来说，就是它返回了一个指向新分配的类型为T的零值的指针。
	//make(T, args)函数的目的与new(T)不同。它仅用于创建slice、map和chan（消息管道），并返回类型T（不是*T）的一个被初始化了的（不是零）实例。
	//  这种差别的出现是由于这三种类型实质上是对在使用前必须进行初始化的数据结构的引用。
	//  例如，slice是一个具有三项内容的描述符，包括指向数据（在一个数组内部）的指针、长度以及容量，在这三项内容被初始化之前，slice值为nil。
	//  对于slice、map和channel，make初始化了其内部的数据结构并准备了将要使用的值。
	...
	*p = make([]int, 10, 10)
	...
	// 习惯用法:
	v := make([]int, 10) // 切片v现在是对一个新的有10个整数的数组的引用
